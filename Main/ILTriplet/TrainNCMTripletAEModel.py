...
# Import Models
from Models.ILTriplet.TripletAEModel import *
...
DISPLAY_FREQUENCY   = 500;         INFO_DISPLAY = '\r%sLearning rate = %f - Epoch = %d - Iter = %d - Feat Cost = %f - Recon Cost = %f'
SAVE_FREQUENCY      = 2500
...
TSBOARD_PATH      = SETTING_PATH + 'Tensorboard/TripletAE/'
RECORD_PATH       = SETTING_PATH + 'TripletAEModel_Record.ckpt'
STATE_PATH        = SETTING_PATH + 'TripletAEModel_CurrentState.ckpt'
BEST_PREC_PATH    = SETTING_PATH + 'TripletAEModel_Prec_Best.ckpt'
VISU_KNOWN_PATH   = SETTING_PATH + 'TripletAEModel_known_class.jpg'
VISU_UNKNOWN_PATH = SETTING_PATH + 'TripletAEModel_unknown_class.jpg'
VISU_ALL_PATH     = SETTING_PATH + 'TripletAEModel_all_class.jpg'

#  GLOBAL VARIABLES
dataset       = None
TripletAE_model = None
TB_handler    = None
...
#                                                                                                                      #
########################################################################################################################
def _create_TripletAE_model():
    global TripletAE_model
    TripletAE_model = TripletAEModel()
...
def _gen_aug_data(_data):
    _reshaped_data     = (_data.reshape((len(_data), 28, 28, 1)) * 255).astype('uint8')
    _train_batch_aug_x = data_aug_manager.augment_images(_reshaped_data)
    _train_batch_aug_x = numpy.concatenate((_train_batch_aug_x, _reshaped_data), axis=0)
    _train_batch_aug_x = _shuffle_data([_train_batch_aug_x])[0]
    _train_batch_aug_x = _train_batch_aug_x[:BATCH_SIZE, ]

    _train_batch_aug_x = _train_batch_aug_x.astype('float32') / 255.
    _train_batch_aug_x = _train_batch_aug_x.reshape((len(_train_batch_aug_x), 1, 28, 28))
    return _train_batch_aug_x
...
def _train_test_model(_all_path,
    ...
    # ----- Temporary record -----
    _cost_train_temp       = []
    _feat_cost_train_temp  = []
    _recon_cost_train_temp = []
    _ratios                = []
    _num_batch_trained     = len(_train_known_data_x) // BATCH_SIZE

    # ----- Train -----
    while (_epoch < NUM_EPOCH):
        _epoch = _iter // _num_batch_trained
        ...
        for _id_batch_trained_data in range(_num_batch_trained_data):
            ...
            # --- Augment data ---
            _train_batch_aug_x = _gen_aug_data(_train_batch_x)

            _iter += 1
            _result = TripletAE_model.feat_train_func(_session     = _session,
                                                      _state         = TRAIN_STATE,
                                                      _learning_rate = _learning_rate,
                                                      _batch_size    = BATCH_SIZE,
                                                      _batch_x       = _train_batch_x,
                                                      _alpha         = ALPHA,
                                                      _batch_recon_x = _train_batch_aug_x)
            # Temporary save info
            _cost_train_temp.append(_result[0] + _result[1])
            _feat_cost_train_temp.append(_result[0])
            _recon_cost_train_temp.append(_result[1])
            _ratios.append(_result[2])
            ...
            if _iter % DISPLAY_FREQUENCY == 0:
                ...
                print (INFO_DISPLAY % ('|-- ', _learning_rate, _epoch, _iter, numpy.mean(_feat_cost_train_temp), numpy.mean(_recon_cost_train_temp)))
                ...
                TB_hanlder.log_scalar(_name_scope = 'Train',
                                      _name       = 'Feat cost',
                                      _value      = numpy.mean(_feat_cost_train_temp),
                                      _step       = _iter)
                TB_hanlder.log_scalar(_name_scope = 'Train',
                                      _name       = 'Reconstruct cost',
            ...
            if _iter % VISUALIZE_FREQUENCY == 0:
                ...
                _recon_imgs = TripletAE_model.recon_func(_session       = _session,
                                                         _state         = TRAIN_STATE,
                                                         _batch_recon_x = _train_batch_aug_x)[0]
                ...
                _origin_set_x = _train_batch_aug_x.reshape((len(_train_batch_aug_x), 28, 28))
...
def _evaluate_model(_all_path,
    ...
    if check_file_exist(_state_path, _throw_error = False):
        ...
        TripletAE_model.load_model(_file)
        _file.close()
    print ('|-- Load best model ! Completed !')

    # # ===== Evaluate dataset =====
    # _prec_unknown_set = _valid_model(_model=TripletAE_model,
    ...
    #
    # _prec_known_set = _valid_model(_model=TripletAE_model,
    #                                _valid_data=[dataset.known_set_x,
    #                                             dataset.known_set_y])
    # _file.writelines('Precision of Known set = %f \n' % (_prec_known_set))
    #
    # _prec_all_set = _valid_model(_model=TripletAE_model,
    ...
    _visualize_model(_model      = TripletAE_model,
                     _test_set_x = _valid_unknown_data_x,
                     _test_set_y = _valid_unknown_data_y,
...
def _draw_heatmap(_all_path,
    ...
    for _label in _all_labels:
        ...
        _feature   = _extract_feature(TripletAE_model, _sub_set_x)
